...
class ProjectExecutor:
    ...
    def _has_structured_login_success_checkpoint(
        self, progress_log: List[ProgressLogEntry]
    ) -> bool:
        for entry in progress_log or []:
            ...
            tool_input = entry.tool_input or {}
            title = str(tool_input.get("title") or "").strip().lower()
            if "login pfm map (interactive)" not in title:
                continue
            status = str(tool_input.get("login_phase_status") or "").strip().lower()
            if status == "success" or tool_input.get("login_success") is True:
                return True
        return False

    def _should_synthesize_login_success_checkpoint(
        self, progress_entries: List[ProgressLogEntry]
    ) -> bool:
        """Recover Phase 2 when a provider turn dies after proving login.

        This deliberately requires both agent login-success narration and concrete
        authenticated-page evidence from browser/tool output; narration alone is
        not enough to unlock the EAD Feature Map.
        """
        saw_success_narration = False
        saw_authenticated_evidence = False
        ...
        for entry in progress_entries or []:
            text = str(entry.text or "").strip().lower()
            if not text:
                continue
            if any(marker in text for marker in failure_markers):
                ...
                saw_authenticated_evidence = False
                continue
            if any(marker in text for marker in login_form_markers) and not any(
                marker in text for marker in authenticated_markers
            ):
                saw_success_narration = False
                saw_authenticated_evidence = False
                continue
            if any(marker in text for marker in success_markers):
                saw_success_narration = True
            if any(marker in text for marker in authenticated_markers):
                if not any(marker in text for marker in login_form_markers):
                    saw_authenticated_evidence = True

        return saw_success_narration and saw_authenticated_evidence

    def _maybe_append_synthesized_login_success_checkpoint(
        self,
        execution_id: str,
        progress_log: List[ProgressLogEntry],
        progress_entries: List[ProgressLogEntry],
    ) -> List[ProgressLogEntry]:
        if self._has_structured_login_success_checkpoint(progress_log):
            return progress_log
        if not self._should_synthesize_login_success_checkpoint(progress_entries):
            return progress_log

        evidence_entry = next(
            (
                entry for entry in reversed(progress_entries)
                if any(
                    marker in str(entry.text or "").lower()
                    for marker in (
                        "#/homepage",
                        "#/banner",
                        "home tz",
        ...
        if evidence_entry:
            for url in (evidence_entry.thumbnail_url, evidence_entry.image_url):
                if url and str(url) not in thumbnail_urls:
                    thumbnail_urls.append(str(url))
        ts = max([entry.ts for entry in progress_entries] or [time.time()]) + 0.001
        checkpoint = ProgressLogEntry(
            ts=ts,
            kind="tool_use",
            text=(
                "Login PFM map (Interactive) | Login confirmed from authenticated "
                "page evidence | Starting Phase 2 exploration."
            ),
            ...
            tool_input={
                ...
                "observation": (
                    "Recovered login checkpoint from authenticated browser evidence "
                    "after the provider turn was interrupted."
                ),
                "finding": "Existing-account sign-in reached an authenticated application page.",
                "reasoning": (
                    "The transcript contains both login-success narration and browser "
                    "output from an authenticated route, so Phase 2 can start safely."
                ),
                "decision": "Mark login as confirmed and continue initialization.",
                "next_direction": "Review inherited PFM context, then begin active feature discovery.",
                "recovered_by_gateway": True,
            },
            image_url=thumbnail_urls[0] if thumbnail_urls else None,
            thumbnail_url=thumbnail_urls[0] if thumbnail_urls else None,
        )
        logger.info(
            "[executor] Synthesized login success checkpoint for execution %s from authenticated evidence",
            execution_id,
        )
        return progress_log + [checkpoint]
    ...
    async def _extract_progress(self, execution_id: str) -> None:
        ...
        if progress_entries:
            ...
            updated_log = self._maybe_append_synthesized_login_success_checkpoint(
                execution_id,
                updated_log,
                progress_entries,
            )
