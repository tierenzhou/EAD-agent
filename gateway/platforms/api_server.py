...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._project_handlers: Optional[Any] = None  # Lazy-init ProjectHandlers
        self._chat_control_handlers: Optional[Any] = None  # Lazy-init ChatControlHandlers
        self._agent_pool: Optional[Any] = None  # Lazy-init SessionAgentPool
    ...
    def _extract_output_items(self, result: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Converts Hermes messages into OpenResponses output items:
        - ``function_call`` items for each pending tool call
    ...
    async def connect(self) -> bool:
        ...
        try:
            mws = [
                mw
                for mw in (cors_middleware, body_limit_middleware, security_headers_middleware)
                if mw is not None
            ]
            ...
            try:
                from projects.agent_pool import SessionAgentPool
                ...
                if hasattr(self, "_project_store") and self._project_handlers:
                    self._executor = ProjectExecutor(
